...
class ItemController extends Controller
{
    ...
    public function show(Item $item, Request $request)
    {
        ...
        if ($userId) {
            $relations['personalPrices'] = function ($query) use ($userId, $selectedServerId) {
            ...
            $relations['recipe.ingredients.personalPrices'] = function ($query) use ($userId, $selectedServerId) {
                $query->where('user_id', $userId);
                if ($selectedServerId) {
                    $query->where('server_id', $selectedServerId);
                }
            ...
            $relations['pricePreferences'] = function ($query) use ($userId, $selectedServerId) {
                $query->where('user_id', $userId);
                if ($selectedServerId) {
                    $query->where('server_id', $selectedServerId);
                }
            };
            $relations['recipe.ingredients.pricePreferences'] = function ($query) use ($userId, $selectedServerId) {
                $query->where('user_id', $userId);
                if ($selectedServerId) {
                    $query->where('server_id', $selectedServerId);
                }
    ...
    public function calculateRecursiveCost(Item $item, Request $request)
    {
        ...
        $calculated = [];
        $craftCost = $item->recipe->calculateCost($server, $calculated, $request->user());

        // Construire l'arbre de craft détaillé
        $craftTree = $this->buildCraftTree($item, $server, $calculated, $request->user());

        // Obtenir le prix direct
        $directPrice = $item->getPriceForServer($server, $request->user());
    ...
    private function buildCraftTree($item, $server, &$calculated = [], $user = null)
    {
        if (! $item->recipe) {
            return null;
        }
        ...
        foreach ($item->recipe->ingredients as $ingredient) {
            ...
            $ingredientCost = $this->getOptimalIngredientCost($ingredient, $server, $calculated, $ingredientData, $user);
    ...
    private function getOptimalIngredientCost($ingredient, $server, &$calculated, &$ingredientData, $user = null)
    {
